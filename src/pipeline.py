...
# Allow running this file directly from the `src/` directory by ensuring the
...
class RunConfig:
    ...
    answering_model: str = "qwen-flash" # gpt-4o-mini-2024-07-18 or "gpt-4o-2024-08-06"
    config_suffix: str = ""
...
class Pipeline:
    ...
    def batch_ingest_reports(self, skip_existing_markdown: bool = True, skip_existing_vectordb: bool = True):
        """
        Batch: PDF -> Markdown
        skip_existing_markdown: skip PDF conversion if markdown already exists
        skip_existing_vectordb: skip embedding work if corresponding faiss already exists
        """
        pdf_paths = list(self.paths.pdf_reports_dir.glob("*.pdf"))
        if not pdf_paths:
            print(f"No PDFs found: {self.paths.pdf_reports_dir}")
            return

        # 1) PDF -> Markdown
        for pdf_path in pdf_paths:
            md_path = self.paths.reports_markdown_path / f"{pdf_path.stem}.md"
            if skip_existing_markdown and md_path.exists():
                print(f"skip: {md_path.name} already exists")
                continue
            self.export_reports_to_markdown(pdf_path.name)

        # 2) Markdown -> chunked_reports
        self.chunk_reports(skip_existing=skip_existing_markdown)

        # 3) chunked_reports -> vector_dbs
        self.create_vector_dbs(skip_existing=skip_existing_vectordb)


    def chunk_reports(self, include_serialized_tables: bool = False, skip_existing: bool = True):
        """
    ...
    def create_vector_dbs(self, skip_existing: bool = True):
        """从分块报告创建向量数据库"""
        input_dir = self.paths.documents_dir
        output_dir = self.paths.vector_db_dir
        
        vdb_ingestor = VectorDBIngestor()
        vdb_ingestor.process_reports(input_dir, output_dir, skip_existing=skip_existing)
...
max_config = RunConfig(
    ...
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = qwen-max",
...
if __name__ == "__main__":
    ...
    # Batch: PDF -> Markdown
    #pipeline.batch_ingest_reports(skip_existing_markdown=True)
